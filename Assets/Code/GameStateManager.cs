...
public class GameStateManager : MonoBehaviour
{
	...
	{
		if (phase.Equals("Placement")) SwapState (new PlacementState(this));
		else if (phase.Equals("Movement")) SwapState (new SelectPieceToMoveState(this));
		else if (phase.Equals("Pushing")) SwapState (new SelectPieceToPushState(this));
		else if (phase.Equals("Ended")) SwapState (null);
	}
	...
	private void Clicked(Coords location)
	{
		...
		if (MenuChoice.Mode == GameMode.AI && Game.Controlling != Game.Master().Turn.TurnPlayer) return;
	...
	private void Update()
	{
		if (State == null) return;
		if (MenuChoice.Mode == GameMode.AI && Game.Controlling != Game.Master().Turn.TurnPlayer) return;
		if (Input.GetKeyDown(KeyCode.Space)) State.Skipped();	
	}
	...
	public class MovePieceState : UnityGameState
	{
		...
		public MovePieceState(GameStateManager context, Coords location) : base(context) { LocationToMove = location; }
	...
	public class PushPieceState : UnityGameState
	{
		...
		public PushPieceState(GameStateManager context, Coords location) : base(context) { PieceToPush = location; }
